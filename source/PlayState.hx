...
class PlayState extends MusicBeatState
{
	...
	var foregroundFlxSprites:FlxTypedGroup<FlxSprite>;
	...
	override public function create()
	{
		...
		foregroundFlxSprites = new FlxTypedGroup<FlxSprite>();

		switch (curStage)
		{
			case "studioBG":
				studio = new FlxSprite(-1470, -800).loadGraphic(Paths.image("backgrounds/studioBG/stagegravey", "shared"));
				add(studio);
				studio.setGraphicSize(Std.int(studio.width * 0.8));
				studio.scrollFactor.set(1, 1);

				sign = new FlxSprite(-1015, -659);
				sign.frames = Paths.getSparrowAtlas("backgrounds/studioBG/BackgroundJoeyDrew", "shared");
				sign.animation.addByPrefix("idle", "final", 24, true);
				sign.animation.play("idle");
				sign.scale.set(0.39, .40);
				add(sign);
				sign.scrollFactor.set(1, 1);

				crate = new FlxSprite(-1470, -800).loadGraphic(Paths.image("backgrounds/studioBG/crate", "shared"));
				crate.setGraphicSize(Std.int(crate.width * 0.8));
				foregroundFlxSprites.add(crate);
				crate.scrollFactor.set(1, 1);

				pillar = new FlxSprite(-1470, -800).loadGraphic(Paths.image("backgrounds/studioBG/Pillar", "shared"));
				pillar.setGraphicSize(Std.int(pillar.width * 0.8));
				foregroundFlxSprites.add(pillar);
				pillar.scrollFactor.set(1, 1);

				lights = new FlxSprite(-1470, -800).loadGraphic(Paths.image("backgrounds/studioBG/lighta", "shared"));
			...
			case "easterEGG":
				...
				lights = new FlxSprite(-1275, -655).loadGraphic(Paths.image("backgrounds/easterEGG/lightmeat", "shared"));
				lights.setGraphicSize(Std.int(lights.width * 0.8));
				foregroundFlxSprites.add(lights);
				lights.scrollFactor.set(1, 1);
		...
		var scorebar:BGSprite = new BGSprite('bendyUI/scorebar', -202, 0);
		scorebar.setGraphicSize(Std.int(scorebar.width * 0.5));
		add(scorebar);
		...
		var inkBar:BGSprite = new BGSprite('bendyUI/inkbarNew', -55, 540);
		inkBar.setGraphicSize(Std.int(scorebar.width * 0.5));
		add(inkBar);
		...
		scoreTxt = new FlxText(0, healthBarBG.y + -607, FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("soupofjustice.ttf"), 20, 0xFFD06119, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		botplayTxt.setFormat(Paths.font("soupofjustice.ttf"), 32, 0xFFD06119, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		lime.app.Application.current.window.title = "Bendy's Genocide - " + SONG.song;
	...
	override public function update(elapsed:Float)
	{
		...
		if (FlxG.keys.anyJustPressed(debugKeysCharacter) && !endingSong && !inCutscene) {
			...
			lime.app.Application.current.window.title = "Bendy's Genocide";
		...
		if (generatedMusic && SONG.notes[curSection] != null) //camera move code
		{
			...
			var kys:FlxPoint = FlxPoint.get(FlxMath.lerp(dadPos.x, bfPos.x, ratio), FlxMath.lerp(dadPos.y, bfPos.y, ratio));
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if(!ClientPrefs.getGameplaySetting('practice', false) && !ClientPrefs.getGameplaySetting('botplay', false)) {
						BendyStoryMenuState.weekCompleted.set(storyWeek, true);
						...
						FlxG.save.data.weekBendyCompleted = BendyStoryMenuState.weekCompleted;
						FlxG.save.flush();
					}
